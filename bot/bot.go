...
// NewBot 创建机器人
func NewBot(token string, proxy string, nodeId string) *Bot {
...
func (b *Bot) StartBot() error {
	...
	if err != nil {
		...
		for i := 0; i < maxRetries; i++ {
			...
			logs.I().Error("WebSocket客户端连接失败: "+b.Proxy, zap.Error(err))
			logs.I().Info("重试连接中... ", zap.Int("重试次数", i+1), zap.Duration("重试间隔", backoff))
		}
		if err != nil {
			logs.I().Error("WebSocket客户端连接失败: ", zap.Error(err))
			return err
		}
	}
...
func (b *Bot) writeMessage() {
	for {
		...
		case msg, ok := <-b.writeChan:
			...
			//logs.I().Info("发送消息: ", zap.Any("message", msg))
		}
	}
}
...
func (b *Bot) receiveMessage() {
	for {
		...
		//logs.I().Info("收到消息: ", zap.String("message", string(message)))
		if err != nil {
